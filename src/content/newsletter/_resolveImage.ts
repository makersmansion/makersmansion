const allImages = import.meta.glob<{ default: ImageMetadata }>(
  "/src/content/newsletter/_images/**/*.{png,jpg,jpeg,webp}",
)

export async function resolveNewsletterImage(url: string | undefined) {
  if (!url) return

  if (!(url in allImages)) {
    throw new Error(`[newsletter] Image "${url}" not found! Is there a typo?`)
  }
...
export async function resolveCoverImage(entry: {
  ...
  if (!(entry.data.coverImage in allImages)) {
    throw new Error(
      `[newsletter] Cover image for "${entry.data.title}" not found! Provided: "${entry.data.coverImage}", is there a typo?`,
    )
  }
...
export async function resolveSocialImage(entry: {
  ...
  if (!(entry.data.socialImage in allImages)) {
    throw new Error(
      `[newsletter] Social image for "${entry.data.title}" not found! Provided: "${entry.data.socialImage}", is there a typo?`,
    )
  }
